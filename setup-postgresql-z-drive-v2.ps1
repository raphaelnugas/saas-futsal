...
$testResult = Execute-PostgreSqlCommand "SELECT version();" "postgres" "Testando conexão"
...
$createDbResult = Execute-PostgreSqlCommand "CREATE DATABASE futsal_nautico;" "postgres" "Criando banco de dados futsal_nautico"
...
Execute-PostgreSqlCommand "\dt" "futsal_nautico" "Listando tabelas"

# Verificar dados
Write-Host ""
Write-Host "=== Verificando dados inseridos ===" -ForegroundColor Green
Execute-PostgreSqlCommand "SELECT COUNT(*) as total_players FROM players;" "futsal_nautico" "Total de jogadores"
Execute-PostgreSqlCommand "SELECT COUNT(*) as total_matches FROM matches;" "futsal_nautico" "Total de partidas"
